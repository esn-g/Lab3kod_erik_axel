function rocket_simulation()
    ...
    angle_guess = angle_initial; % Initial guess for launch angle
...
%% b)
  ...
  %% c)
    % Newton-Raphson Shooting Method for finding launch angle
    angle_tolerance = deg2rad(0.01); % Tolerance for final position
    max_iterations = 100; % Maximum iterations
    for iter = 1:max_iterations
        % Run RK4 integration with current launch angle
        [posX, ~, ~, ~] = RK4_positions_second_order(@accel_func, vx0, vy0, pos_x0, pos_y0, h, N);

        % Final x position
        final_x = posX(end);

        % Check if final position is close enough to target
        if abs(final_x - x_target) <= angle_tolerance
            fprintf('Angle found: %f degrees\n', rad2deg(angle_guess));
            break;
        end

        % Numerical derivative approximation
        delta_angle = deg2rad(0.1); % Small angle increment
        angle_derivative = (rocket_simulation_derivative(angle_guess + delta_angle, x_target) - rocket_simulation_derivative(angle_guess, x_target)) / delta_angle;

        % Newton-Raphson update for launch angle
        angle_guess = angle_guess - (final_x - x_target) / angle_derivative;

        % Update initial conditions with new launch angle
        vx0 = cos(angle_guess) * 20;
        vy0 = sin(angle_guess) * 20;
    end
...
function derivative = rocket_simulation_derivative(angle, x_target)
    % Run RK4 integration to compute derivative
    vx0 = cos(angle) * 20; % Initial velocity x-component
    vy0 = sin(angle) * 20; % Initial velocity y-component
    pos_x0 = 0; % Initial position x-component
    pos_y0 = 0; % Initial position y-component
    ...
    % Derivative
    derivative = (final_x - x_target) / angle;
end
